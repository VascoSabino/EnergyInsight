filename = 'eta_fu_sector_27_countries_X.csv';

% Read the data from the CSV file
data_Xsector = readcell(filename, 'Delimiter', ',');
headerRow = data_Xsector(1, :);
...
variableValues = string(data_Xsector(:, variableColumnIndex));
undesiredIndices = ~strcmp(variableValues, variableName);
data_Xsector(undesiredIndices, :) = [];
...
countries = string(data_Xsector(2:end, countryColumnIndex));
...
sectors = string(data_Xsector(:, sectorColumnIndex));
...
stages = string(data_Xsector(:, varColumnIndex));
selectedStages = stages(countryIndices);

% Find the column indices to the right of the 'var' column
columnIndices = varColumnIndex + 1:size(data_Xsector, 2);

% Create new variables based on the sectors and conversion stages
for i = 1:length(selectedSectors)
    if selectedCountry == uniqueCountries(1)
        variableName = ['X_' char(selectedSectors(i)) '_' char(selectedStages(i))];
        variableData = data_Xsector(i, varColumnIndex+1:end);
    else
        variableName = ['X_' char(selectedSectors(i)) '_' char(selectedStages(i))];
        variableData = data_Xsector(i-2+min(Indices), varColumnIndex+1:end);
    end
...
X_vectorsname_Final = who('*_Final');
numSectors = numel(X_vectorsname_Final);

% Initialize the integer 'total final energy from all years'
XtotalFinal = 0;
XsectorFinal = zeros(numSectors, length(years));
legendNamesFinal = cell(numSectors, 1);
legendNamesFinal(:) = {''};

% Calculate the total final energy for each sector and create stacked bar plot
for i = 1:numSectors
    XsectorFinal(i, :) = eval(X_vectorsname_Final{i});
    XtotalFinal = XtotalFinal + sum(XsectorFinal(i, :));
end


figure;
hBarFinal = bar(years, XsectorFinal, 'stacked');
numSectors = size(XsectorFinal, 1);
...
for j = 1:numSectors
    if sum(XsectorFinal(j, :)) >= Sector_minWeight * XtotalFinal
        cleanName = strrep(X_vectorsname_Final{j}, 'X_', '');
...
title('Final exergy by sector - ' + desiredCountry);
ylabel('[ktoe]');
...
% Create pie chart
sectorTotalFinal = sum(XsectorFinal, 2);
figure;
hPieFinal = pie(sectorTotalFinal, legendNamesFinal);
title(sprintf('Final exergy by sector (%d-%d) - %s', min(years), max(years), desiredCountry));
...
percentageValuesFinal = 100 * sectorTotalFinal / XtotalFinal;
legendLabelsFinal = cell(numSectors, 1);
hTextFinal = findobj(hPieFinal, 'Type', 'text');
for i = 1:numSectors
    if sum(XsectorFinal(i, :)) >= Sector_minWeight * XtotalFinal
...
X_vectorsname_Useful = who('*_Useful');
numSectors = numel(X_vectorsname_Useful);

% Initialize the integer 'total useful energy from all years'
XtotalUseful = 0;
XsectorUseful = zeros(numSectors, length(years));
legendNamesUseful = cell(numSectors, 1);
legendNamesUseful(:) = {''};

% Calculate the total useful energy for each sector and create stacked bar plot
for i = 1:numSectors
    XsectorUseful(i, :) = eval(X_vectorsname_Useful{i});
    XtotalUseful = XtotalUseful + sum(XsectorUseful(i, :));
end
...
figure;
hBarUseful = bar(years, XsectorUseful, 'stacked');
numSectors = size(XsectorUseful, 1);
colorsUseful = colorcube(numSectors*3);  % Define a colormap with the same number of colors as segments

% Create legend labels and set bar colors
for j = 1:numSectors
    if sum(XsectorUseful(j, :)) >= Sector_minWeight * XtotalUseful
        cleanName = strrep(X_vectorsname_Useful{j}, 'X_', '');
...
title('Useful exergy by sector - ' + desiredCountry);
ylabel('[ktoe]');
...
% Create pie chart
sectorTotalUseful = sum(XsectorUseful, 2);
figure;
hPieUseful = pie(sectorTotalUseful, legendNamesUseful);
title(sprintf('Useful exergy by sector (%d-%d) - %s', min(years), max(years), desiredCountry));
...
percentageValuesUseful = 100 * sectorTotalUseful / XtotalUseful;
legendLabelsUseful = cell(numSectors, 1);
hTextUseful = findobj(hPieUseful, 'Type', 'text');
for i = 1:numSectors
    if sum(XsectorUseful(i, :)) >= Sector_minWeight * XtotalUseful
